...
def get_post_login_redirect(request):
...
class DiscordSignInView(RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        if not has_discord_configuration():
            messages.error(self.request, ERROR_DISCORD_MISSING_CONFIGURATION)
            return resolve_url(LOGIN_ROUTE_NAME)

        store_next_url(self.request)
...
class DiscordCallbackView(View):
    def get(self, request, *args, **kwargs):
        if request.GET.get("error"):
            messages.error(request, ERROR_DISCORD_ACCESS_FAILED)
            return redirect(LOGIN_ROUTE_NAME)

        if not self._has_valid_state(request):
            messages.error(request, ERROR_DISCORD_INVALID_STATE)
            return redirect(LOGIN_ROUTE_NAME)

        code = request.GET.get("code")
        access_token = exchange_code_for_token(request, code)
        if not access_token:
            messages.error(request, ERROR_DISCORD_TOKEN_FAILED)
            return redirect(LOGIN_ROUTE_NAME)

        guild_member = get_discord_guild_member(access_token)
        if settings.DISCORD_GUILD_ID and (not guild_member or not guild_member.get("user")):
            messages.error(request, ERROR_DISCORD_MEMBER_REQUIRED)
            return redirect(LOGIN_ROUTE_NAME)

        base_user_data = get_discord_user(access_token)
        if not base_user_data:
            messages.error(request, ERROR_DISCORD_PROFILE_FAILED)
            return redirect(LOGIN_ROUTE_NAME)

        backend = DiscordAuthenticationBackend()
        discord_user = backend.authenticate(request, user=self._build_user_payload(base_user_data, guild_member))
        if not discord_user:
            error_msg = getattr(request, DISCORD_LOGIN_ERROR_ATTR, ERROR_DISCORD_AUTH_FAILED)
            messages.error(request, error_msg)
            return redirect(LOGIN_ROUTE_NAME)

        login(request, discord_user, backend=DISCORD_AUTHENTICATION_BACKEND)
        return redirect(get_post_login_redirect(request))

    @staticmethod
    def _build_user_payload(base_user_data, guild_member):
        return {
            ...
            "nick": (guild_member or {}).get("nick") or base_user_data.get("global_name") or base_user_data.get("username"),
